...
task = VCtx('''backtest
    start: 2018-01-20 00:00:00
    end: 2018-03-22 12:00:00
    period: 15m
    exchanges: [{"eid":"OKEX","currency":"LTC_BTC","balance":3,"stocks":0}]
''') # initialize backtest engine from __doc__
...
def get_trade_signal(n, k1, k2):
    ...
    # Log(bar_list[-2]["Open"], rg, upper_threshold, lower_threshold)
...
def execute_old(tar_pos):
    ...
    if len(position_list) == 0:
        ...
        if tar_pos > 0:
            ...
            exchange.Buy(g_tick.Sell, tar_pos)
        elif tar_pos < 0:
            exchange.SetDirection("sell")
            exchange.Sell(g_tick.Buy, -tar_pos)
    else:
        # 如果非空仓，根据tar pos平仓
        pos_long = 0
        pos_short = 0
...
def execute(tar_pos):
    ...
    @return: 
    '''
    pos = exchange.GetAccount()['Stocks']
    if pos == tar_pos:
        return 
    print(pos, tar_pos)
    # Log(position)
    if pos == 0:
        ...
        # 如果非空仓，根据tar pos平仓
        if pos > 0 and tar_pos <= 0:
            exchange.SetDirection("closebuy")
            exchange.Sell(g_tick.Buy, -tar_pos)
        elif pos < 0 and tar_pos >= 0:
            exchange.SetDirection("closesell")
            exchange.Buy(g_tick.Sell, tar_pos)
    return 
...
def loop():
    ...
    while 1:
        ...
        if now_time.date() != signal_date:
            # 生成开平仓信号
            open_flag, close_flag = get_trade_signal(4, 0.5, 0.5)
